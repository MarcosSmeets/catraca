...
// ListTicketsUseCase lists all tickets for a user.
...
func (uc *ListTicketsUseCase) Execute(ctx context.Context, userID uuid.UUID) ([]*entity.Ticket, error) {
	return uc.ticketRepo.ListByUserID(ctx, userID)
}

// GetTicketUseCase fetches a single ticket, enforcing ownership via order link.
...
func (uc *GetTicketUseCase) Execute(ctx context.Context, ticketID, userID uuid.UUID) (*entity.Ticket, error) {
